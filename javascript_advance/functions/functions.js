...
function sleepIn(weekday, vacation) {
  let return_statement = false
  let weekdays = ["monday", "tuesday", "wednesday", "thursday", "friday"]
  if (((typeof(weekday) == "string") && !(weekdays.includes(weekday.toLowerCase()))) || vacation) {
...
function luckySum(a, b, c) {
  ...
  //Code Goes Here
  let total_count = 0
  let a_lucky = a !== 13 ? true : false
  let b_lucky = b !== 13 ? true : false
  let c_lucky = c !== 13 ? true : false
...
function caught_speeding(speed, is_birthday) {
  //Code Goes Here
  let return_statement = "no_ticket"
  let punishment = {
    0: "no_ticket",
    1: "small_ticket",
    2: "big_ticket"
  ...
  if (speed >= 81){
    return_statement = punishment[2]
  } else if (speed >=61){
    return_statement = punishment[1]
  }
...
function makeBricks(small, big, goal) {
  //Code Goes Here
  let reminder = goal % 5
  let division = (goal - reminder) / 5
  let return_statement = ((division <= big) && (reminder <= small)) ? true : false
  return return_statement
}
